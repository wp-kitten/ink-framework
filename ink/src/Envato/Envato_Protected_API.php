...
class Envato_Protected_API
{
	...
	{
		if ( $user_name == '' ) {
			$this->set_error( 'user_name', __( 'Please enter your Envato Marketplace Username.', 'ink-fw' ) );
		}
		if ( $api_key == '' ) {
			$this->set_error( 'api_key', __( 'Please enter your Envato Marketplace API Key.', 'ink-fw' ) );
		}
	...
	{
		if ( $set == '' ) {
			$this->set_error( 'set', __( 'The API "set" is a required parameter.', 'ink-fw' ) );
		}
		if ( $user_name == '' ) {
	...
	{
		if ( ! isset( $item_id ) ) {
			$this->set_error( 'item_id', __( 'The Envato Marketplace "item ID" is a required parameter.', 'ink-fw' ) );
		}
	 ...
	 * @return    object|bool|array      Details for the given item as object, errors or false on error.
	 *
	 * @access    public
	 * @since     1.0
	 * @updated   1.3
	 ...
	 * @access    private
	 * @since     1.0
	 * @return bool|object The results of the wp_remote_request request or bool false on error
	 */
